...
def show_scatter_and_results(df, labels=["X Mean", "Y Mean", "X SD", "Y SD", "Corr."]):
    ...
    plt.text(110, y_off + 80, labels[0].ljust(max_label_length) + ": " + format(res[0], "0.9f")[:-2], fontsize=fs,
             alpha=0.3)
    plt.text(110, y_off + 65, labels[1].ljust(max_label_length) + ": " + format(res[1], "0.9f")[:-2], fontsize=fs,
             alpha=0.3)
    plt.text(110, y_off + 50, labels[2].ljust(max_label_length) + ": " + format(res[2], "0.9f")[:-2], fontsize=fs,
             alpha=0.3)
    plt.text(110, y_off + 35, labels[3].ljust(max_label_length) + ": " + format(res[3], "0.9f")[:-2], fontsize=fs,
             alpha=0.3)
    plt.text(110, y_off + 20, labels[4].ljust(max_label_length) + ": " + format(res[4], "+.9f")[:-2], fontsize=fs,
             alpha=0.3)

    plt.text(110, y_off + 80, labels[0].ljust(max_label_length) + ": " + format(res[0], ".9f")[:-7], fontsize=fs,
             alpha=1)
    plt.text(110, y_off + 65, labels[1].ljust(max_label_length) + ": " + format(res[1], "0.9f")[:-7], fontsize=fs,
             alpha=1)
    plt.text(110, y_off + 50, labels[2].ljust(max_label_length) + ": " + format(res[2], "0.9f")[:-7], fontsize=fs,
             alpha=1)
    plt.text(110, y_off + 35, labels[3].ljust(max_label_length) + ": " + format(res[3], "0.9f")[:-7], fontsize=fs,
             alpha=1)
    plt.text(110, y_off + 20, labels[4].ljust(max_label_length) + ": " + format(res[4], "+.9f")[:-7], fontsize=fs,
             alpha=1)
...
# 这些是我们为之烦恼的硬编码形状。如果可以用一个工具来绘制这些形状代替会更好
...
# shake:一次交互的最大动作
# 
def run_pattern(df, target, sv_path, iters=100000, num_frames=100, decimals=2, shake=0.2,
...
def do_single_run(start_dataset, target, save_path=None, iterations=100000, decimals=2, num_frames=100):
    global it_count
    global frame_count
    it_count = 0
    frame_count = 0
    ...
    temp = run_pattern(df, target, sv_path=save_path, iters=iterations, num_frames=num_frames)
    return temp
